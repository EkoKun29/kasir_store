    ...
    <table class="table datatable" style="margin-top: 15px">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $dt->kode }}</td>
                <td>
                    <a href="{{ route('audit.show', $dt->id) }}" class="btn btn-primary btn-sm">Detail </a>
                    <button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete({{ $dt->id }})">
                        Hapus
                    </button>
                    
                    <form id="delete-form-{{ $dt->id }}" action="{{ route('audit.delete', $dt->id) }}" method="POST" style="display: none;">
                        @csrf
                        @method('DELETE')
                    </form>
                </td>
